...
export default function PianoKeyboard({ selectedPCs, onToggle }: Props) {
  ...
  const handleKeyDown = (e: React.KeyboardEvent, pc: PitchClass) => {
  ...
  return (
    <div className="bg-gray-800 rounded-lg p-3 sm:p-4" role="group" aria-label="Piano keyboard">
        ...
        {WHITE_KEYS.map((pc, i) => {
          ...
          return (
            <rect
              ...
              strokeWidth={1}
              onClick={() => onToggle(pc)}
              onKeyDown={(e) => handleKeyDown(e, pc)}
              role="button"
              aria-pressed={active}
              aria-label={`${NOTE_NAMES[pc]} ${active ? 'selected' : 'not selected'}`}
              tabIndex={0}
            />
          );
        })}
