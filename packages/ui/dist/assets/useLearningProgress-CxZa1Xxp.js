import{r as t}from"./index-CsQSCGz0.js";const u="intro-set-theory",y="Introduction to Pitch-Class Set Theory",f="Learn how mathematicians and theorists classify musical collections using sets, transposition, inversion, and symmetry groups.",g="🎵",b=[{id:"pitch-classes",title:"What is a Pitch Class?",content:`## What is a Pitch Class?
...
This is Chrometria's unique contribution: taking the 50-year-old Forte catalog and lifting it to a richer description that connects music to the same mathematics used to understand molecular spectra, crystal lattices, and quantum mechanics.`,task:{description:"Find a set with high symmetry in the Atlas and explore its character table entry via the detail page.",link:"#atlas"},quiz:{question:"What information does a character table encode about a symmetry group?",options:["The tempo and meter of compositions that use the group","The traces of representation matrices for each conjugacy class and irreducible representation","The MIDI note numbers for each pitch class","The historical timeline of who discovered each symmetry group"],correctIndex:1,explanation:"A character table encodes the **characters** (matrix traces) of each irreducible representation (Mulliken label) evaluated on each conjugacy class of symmetry operations. This completely characterizes all the group's representations and determines how objects — whether molecular orbitals or pitch-class sets — transform under the group's symmetry operations."}}],k={id:T,title:w,description:_,icon:x,lessons:C},q=[v,k];function z(r){const[h,o]=t.useState(new Set),[m,c]=t.useState(!1),n=t.useCallback(async()=>{if(r){c(!0);try{const e=await fetch("/api/learning/progress",{credentials:"include"});if(e.ok){const s=await e.json();o(new Set(s.progress.map(i=>`${i.path_id}/${i.lesson_id}`)))}}catch{}finally{c(!1)}}},[r]);t.useEffect(()=>{n()},[n]);const d=t.useCallback(async(e,s)=>{try{(await fetch(`/api/learning/progress/${e}/${s}/complete`,{method:"POST",credentials:"include"})).ok&&o(a=>new Set([...a,`${e}/${s}`]))}catch{}},[]),p=t.useCallback(async e=>{try{(await fetch(`/api/learning/progress/${e}`,{method:"DELETE",credentials:"include"})).ok&&o(i=>{const a=new Set(i);for(const l of a)l.startsWith(`${e}/`)&&a.delete(l);return a})}catch{}},[]);return{completedLessons:h,loading:m,markComplete:d,resetPath:p,refresh:n}}export{q as L,z as u};
